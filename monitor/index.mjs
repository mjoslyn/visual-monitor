...
import { captureScreenshot, CloudflareChallengeError } from './screenshot.mjs';
  ...
  for (const site of sites) {
    ...
    try {
      ...
      if (err instanceof CloudflareChallengeError) {
        // Cloudflare challenge — skip this run entirely, keep previous state
        console.warn(`[monitor]   SKIPPED: ${err.message}`);
        state.activityLog.unshift({
          timestamp: now,
          siteId: site.id,
          type: 'skipped',
          detail: 'Cloudflare challenge page detected — skipped this check',
        });
      } else {
