...
export class CloudflareChallengeError extends Error {
  constructor(siteId) {
    super(`Cloudflare challenge page detected for site "${siteId}" — skipping`);
    this.name = 'CloudflareChallengeError';
  }
}

/**
 * Detect Cloudflare browser-verification / challenge pages.
 * Returns true if the current page appears to be a Cloudflare interstitial.
 */
async function isCloudflareChallenge(page) {
  return page.evaluate(() => {
    // Cloudflare Turnstile / managed challenge containers
    if (document.querySelector('#challenge-running, #challenge-stage, #cf-challenge-running')) {
      return true;
    }

    // Cloudflare wraps its challenge in an iframe sourced from challenges.cloudflare.com
    const cfIframe = Array.from(document.querySelectorAll('iframe')).some((f) =>
      (f.src || '').includes('challenges.cloudflare.com')
    );
    if (cfIframe) return true;

    // Check page title — Cloudflare uses "Just a moment..."
    if (document.title.toLowerCase().includes('just a moment')) return true;

    // Check visible body text for common Cloudflare phrases
...
export async function captureScreenshot(site) {
  ...
  try {
    ...
    // Wait out bot verification screens (Cloudflare, etc.)
    if (await isCloudflareChallenge(page)) {
      console.log(`[screenshot] Cloudflare challenge detected for ${site.id}, polling up to 30s...`);
      ...
      if (await isCloudflareChallenge(page)) {
        throw new CloudflareChallengeError(site.id);
      }
      console.log(`[screenshot] Cloudflare challenge cleared for ${site.id}`);
    }
